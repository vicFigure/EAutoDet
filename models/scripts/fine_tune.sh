...
CUDA_VISIBLE_DEVICES=$gpustr python train.py $CONFIG --data $data --cfg $cfg --project $project$CONFIG --name fine_tune --weights '' --train_type $train_type --batch-size $BATCHSIZE > $LOG_DIR/$ID-yaml$yamlID-$train_type$CONFIG.log 2>&1 &
